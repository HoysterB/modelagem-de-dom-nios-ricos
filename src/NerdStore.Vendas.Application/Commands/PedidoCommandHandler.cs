...
namespace NerdStore.Vendas.Application.Commands
{
...

    {
        ...
        public async Task<bool> Handle(AdicionarItemPedidoCommand message,
            CancellationToken cancellationToken)
        {
            if (!ValidarComando(message)) return false;

            var pedido = await _pedidoRepository
                                .ObterPedidoRascunhoPorClienteId(message.ClienteId);
            ...
            else
            {
                ...
                {
                    _pedidoRepository.AtualizarItem(pedido.PedidoItems.FirstOrDefault(
                        p => p.ProdutoId == pedidoItem.ProdutoId));
                }
                else
                {
                ...
                pedido.AdicionarEvento(new PedidoAtualizadoEvent(
                    pedido.ClienteId, pedido.Id, pedido.ValorTotal));
            }
        ...
        private bool ValidarComando(Command message)
        {
            ...
            foreach (var erro in message.ValidationResult.Errors)
            {
                _mediatorHandler.PublicarNotificacao(
                    new DomainNotification(message.MessageType, erro.ErrorMessage));
            }
